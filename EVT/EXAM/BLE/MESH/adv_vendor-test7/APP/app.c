...
static int  vendor_model_srv_send(uint16_t addr, char *pData, uint16_t len);
...
static void vendor_model_srv_rsp_handler(const vendor_model_srv_status_t *val)
{
    ...
    {
        // 收到透传数据
//        APP_DBG("data %d%d%d%d from 0x%04x",
//                val->vendor_model_srv_Event.trans.pdata[0],val->vendor_model_srv_Event.trans.pdata[1],val->vendor_model_srv_Event.trans.pdata[2],val->vendor_model_srv_Event.trans.pdata[3],
...
//        char receive_data=0;
//        uint8_t receive_data_len=0,flag_receive_data=0;
//        receive_data_len=val->vendor_model_srv_Event.trans.len;
//        for(flag_receive_data=0;flag_receive_data<receive_data_len;flag_receive_data++)
//        {
//            receive_data=receive_data+(val->vendor_model_srv_Event.trans.pdata[flag_receive_data])*pow(10,(receive_data_len-flag_receive_data-1));
//        }
        APP_DBG("data=%s from 0x%04x",
                val->vendor_model_srv_Event.trans.pdata,
                val->vendor_model_srv_Event.trans.addr);
        tmos_memcpy(&app_mesh_manage, val->vendor_model_srv_Event.trans.pdata, val->vendor_model_srv_Event.trans.len);
...
//            if(data[0])
//            {
//            status = vendor_model_srv_send(0x0001, data, 9);
//            }
//            if(status)
//            {
...
static uint16_t App_ProcessEvent(uint8_t task_id, uint16_t events)
{
    if(events & APP_NODE_TEST_EVT)
    {
        uint8_t status;
        char data[20];
        if((strlen(plate))&&(distant<=15)) //后面要加上距离判断小于15cm才传输车牌
        //if(strlen(plate))
        {
            sprintf(data, "%s", plate);//广播
            status = vendor_model_srv_send(0x0001,data, 20);
        }
...
//        uint8_t status;
//        char data[7];
//        sprintf(data, "%.2f", distant);
//        status = vendor_model_srv_send(0x0001,data, 7);
//
//        tmos_start_task(App_TaskID, APP_NODE_TEST_EVT, 6400);
//        return (events ^ APP_NODE_TEST_EVT);
    }

...
static int  vendor_model_cli_send(uint16_t addr, char *pData, uint16_t len);
...
static void vendor_model_cli_rsp_handler(const vendor_model_cli_status_t *val)
{
    ...
    if(val->vendor_model_cli_Hdr.opcode == OP_VENDOR_MESSAGE_TRANSPARENT_MSG)
    {
        ...
        APP_DBG("data = %s from 0x%04x",
                val->vendor_model_cli_Event.trans.pdata,
...
static uint16_t App_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & APP_NODE_TEST_EVT)
    {
        if(app_nodes[1].node_addr)
        {
            uint8_t status,flag_dataoutput;

            char data[9] = {"京123456"};
            //char data[8] = {'e','1','2','3','4','5','6'};

            for(flag_dataoutput=1;;flag_dataoutput++)
            {
                if(app_nodes[flag_dataoutput].node_addr==0)
                {
                    APP_DBG("now number of nodes is %d", app_nodes[flag_dataoutput-1].node_addr);
                    break;
                }

                status = vendor_model_cli_send(app_nodes[flag_dataoutput].node_addr, data, 9);
...
//        {
//            uint8_t status;
//            APP_DBG("node is %x", app_nodes[1].node_addr);
//            uint8_t data[4] = {0, 1, 2, 3};
//            status = vendor_model_cli_send(app_nodes[1].node_addr, data, 4); // 调用自定义模型客户端的透传函数发送数据
//            status = vendor_model_cli_send(app_nodes[2].node_addr, data, 4);
//            status = vendor_model_cli_send(app_nodes[3].node_addr, data, 4);
//            if(status)
//                APP_DBG("trans failed %d", status);
//        }
//        tmos_start_task(App_TaskID, APP_NODE_TEST_EVT, 6400);
